        ...
        <div class="col-md-12">
            ...
            <div class="form-group">
               <a href="?type=myArticles">
                <button type="button" id="myArticles" class="btn btn-success btn-sm col-md-12 @if(request()->type == 'myArticles') active @endif"> My Articles @if($myArticles)<span class="label label-primary text-dark"> {{$myArticles}}
                </span>@endif</button>
            </a>
        </div>
    </div>
        ...
        <div class="form-group">
         <a href="?type=editable">
            <button type="button" id="editable" class="btn btn-primary btn-sm col-md-12 @if(request()->type == 'editable') active @endif"> 
                ...
                <span class="label label-primary text-dark"> {{count($editable_articles)}}
                </span>
                @endif
            </button>
        </a>
...
@endif
@if (auth()->id() != $publisher && !auth()->user()->hasRole($inhouseWriterRole))
<div class="col-md-12 p-t-10">
    <div class="form-group">
...
<div class="col-md-12 p-t-10">
    <div class="form-group">
       <a href="?type=paidUnpaidArticles">
        <button type="button" id="paidUnpaidArticles" class="btn btn-success btn-sm col-md-12 @if(request()->type == 'paidUnpaidArticles') active @endif">Paid/Unpaid Articles</button>
    </a>
</div>
</div>
@endif
...
<form>
    <div class="col-md-12 p-t-10">
        ...
        <select class="select2 form-control col-md-6" name="type" id="type" data-style="form-control">
            ...
            <option value="paidUnpaidArticles" @if(request()->type == 'paidUnpaidArticles') selected @endif>Paid/Unpaid Articles</option>
            @endif
            @if(auth()->id() == $publisher || auth()->id() == $writerHead || auth()->user()->hasRole('admin'))
    ...
    <div class="col-md-12 m-t-10 m-b-10">
        <div class="checkbox checkbox-info">
            ...
            <label for="hide-completed-tasks">Hide Completed Writing</label>
        </div>
    </div>
    @if(request()->entries)
...
<div class="row">
    ...
    <div class="col-md-9">
        <form id="order">
            ...
            <input type="hidden" name="entries" value="{{request()->entries ? request()->entries : 10}}">
            @endif
            <label class="col-md-2 text-right">Order By: </label>
            <div class="col-md-3">
...
<table class="table table-bordered table-hover">
    ...
    <tbody id="list">
        ...
        <tr role="row" class="odd">
            <td>{{$article->id}}</td>
            <td>
                <a target="_blank" href="{{route('admin.article.show',$article->id)}}" class="@if($article->writing_status == 1) text-warning @endif" style="@if($article->writing_status == 2 && $article->assignee == auth()->id()) color: rgba(132,128,144,0.8); @elseif($article->publishing == 1 && $article->publishing_status == 1 && $article->writing_status == 2) color: rgba(132,128,144,0.8); @elseif($article->publishing != 1 && $article->writing_status == 2) color: rgba(132,128,144,0.8); @elseif($article->writing_status == 0) color: rgba(0,0,0,1); @endif">{{$article->title}} 

                    @if($article->article_status == 1 && $article->invoice->status != 1) 
                    <label class="label label-primary">Unpaid</label>
                    @elseif($article->article_status == 1 && $article->invoice->status == 1) 
