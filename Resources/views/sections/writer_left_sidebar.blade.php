<div class="navbar-default sidebar" role="navigation">
    ...
    <div class="sidebar-nav navbar-collapse slimscrollsidebar">
        ...
        @if(auth()->id() == $head || auth()->id() == $publisherHead || auth()->user()->hasRole('admin') || auth()->user()->hasRole($writer) || auth()->user()->hasRole($inhouseWriter))
        <li><a href="javascript:;" class="waves-effect"><i class="ti-pencil"></i> <span class="hide-menu"> @lang('article::app.menu.article')<span class="frow"></span> </span></a>
            <ul class="nav nav-second-level collapse">
                ...
                @if(auth()->id() != $publisherHead)
                <li><a href="{{is_null(route('member.article.writers')) ? 'javascript:;' : route('member.article.writers')}}" class="waves-effect">
                    <span class="hide-menu">@if(!auth()->user()->hasRole($writer) && !auth()->user()->hasRole($inhouseWriter)) @lang('article::app.writers') @else Profile @endif</span></a>
                </li>
                @endif
                @if(!auth()->user()->hasRole($inhouseWriter) && auth()->id() != $publisherHead)
                ...
                @endif
                @if(auth()->id() == $head || auth()->user()->hasRole('admin'))
