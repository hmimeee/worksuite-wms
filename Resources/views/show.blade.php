...
<div class="row bg-title">
    ...
    <div class="col-lg-8 col-md-5 col-sm-6 col-xs-12">
        ...
        <a href="{{route('member.article.index')}}" class="btn btn-sm btn-rounded btn-outline m-b-10" style="background: grey; color: white; margin-left: -15px;"><i class="fa fa-arrow-left"></i> Return to Articles</a>
...
<div class="row">
        ...
        <div class="white-box" style="padding: 10px; @if(user()->is_writer_head() && !user()->hasRole('admin') && !user()->is_writer_head_assistant()) height: 100px; @endif"> 
            ...
            <div class="form-group row m-l-5">
                ...
                <div class="col-md-1 col-sm-12">
                    ...
                    <input type="number" name="wordCount" id="wordCount" class="form-control" placeholder="Word Count">
                </div>

                @if($article->publishing ==1)
                <div class="col-md-2">
                    {{-- <label class="control-label required">Publishing Due Date</label> --}}
                    <input type="text" name="publishing_deadline" id="publishing_deadline" class="form-control" autocomplete="off" placeholder="Publishing Due Date">
                </div>
                <div class="col-md-2">
                    {{-- <label class="control-label">Website to Publish</label> --}}
                    <input type="text" name="website" id="website" class="form-control" placeholder="Website to Publish">
                </div>
                @endif
            </div>
            @endif
            ...
            <div class="form-group row m-l-5">
                ...
                <div class="col-md-10">
                    <h4>
                        ...
                        Article review is in progress to the writer
                        <b>{{\App\User::find($article->reviewWriter->value)->name}}</b>
                        <a class="btn btn-xs btn-info" id="updateReviewBtn">Change</a>
                    </h4>
                </div>
    ...
    <div class="col-xs-12">
        <div class="white-box"  style="padding: 1% 5%;"> 
            ...
            @if(!user()->is_writer() && $article->task)
                        ...
                        <div class="col-xs-12 m-b-10" id="comments-list">
                            ...
                            <div class="row m-t-10" style="background: rgba(0,0,0,0.03); padding: 5px;">
                                ...
                                <div class="col-xs-12">
                                    ...
                                    @php($size = file_exists(public_path('/user-uploads/article-comment-files/').$file[$i]) ? filesize(public_path('/user-uploads/article-comment-files/').$file[$i])/1024 : 0)
                                    @if($size < 1024)
                                    ({{number_format($size, 2)}} KB)
                                    @elseif($size > 1024)
                                    ({{number_format($size/1024, 2)}} MB)
                            ...
                            <form id='Comment' method='POST' enctype="multipart/form-data">
                                ...
                                <div class="col-xs-12 m-b-10" id="commentFiles">
                                    <div class="upload-btn-wrapper">
                                        <button class="btn-upload" style="display: block;width: 100%;"><i class="fa fa-paperclip"></i> Drag and Drop Your Files <br>
...
<script>
    $('#updateReviewBtn').click(function(){
        $('#updateReview').toggle('show');
        $(this).toggleClass('btn-info btn-inverse');
       $(this).text( $(this).text() == 'Change' ? 'Cancel' : 'Change');
    })
